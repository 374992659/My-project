...
class ChatMessageController extends VersionController
{
    /*
     * 好友语音/文件/图片消息上传
     * APP参数： imageData(文件流)  type(传递图片时不需传递此参数  语音：voice) ext(传递图片时不需传递此参数     文件后缀名例如：'.mp3','.vma'等)
     * */
    ...
    /*
     * 群语音文件上传
     *  APP参数： imageData(文件流)  type( 语音：voice)  ext (文件后缀名例如：'.mp3','.vma'等)
     * */
    protected function uploadGroupVoiceFile_v1_0_0(){
        if(intval($_GET['is_wap'])!==1){
            $a = $this->pdata['imageData'];
            if ( empty($a) ) return $this->echoEncrypData(1,'没有文件被选中');
            $save_path= APP_PATH . 'Common/Upload/Voice/GroupVoice/' . date(m) . date(d) . '/';
            $res = $this->uploadAppImg($save_path,$a);
            if($res){
                $this->echoEncrypData(0,'',$res);
            }else{
    ...
    protected function uploadGroupFile_v1_0_0(){
        if(intval($_GET['is_wap'])!==1){
            $a = $this->pdata['imageData'];
            if ( empty($a) ) return $this->echoEncrypData(1,'没有文件被选中');
            $save_path= APP_PATH . 'Common/Upload/File/Group/' . date(m) . date(d) . '/';
            $res = $this->uploadAppImg($save_path,$a);
            if($res){
                $this->echoEncrypData(0,'',$res);
            }else{
