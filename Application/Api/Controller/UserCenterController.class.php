...
//        $this->echoEncrypData(900,C('MESSAGE_SIGN'),C('CHANGE_PHONE_CODE'));
        $account_code = $this->account_code;
...
//        }
        ...
        $count = $garden_room->where(['city_id'=>$this->pdata['city_id'],'garden_code'=>$garden_code,'room_num'=>$this->pdata['room_num'],'user_code'=>$this->account_code])->count();
        if($count)$this->echoEncrypData(1,'你在该房间已有其他认证');
        $city_id = substr($this->account_code,0,6);
        $model = new Model\OwnerApplicationController($city_id); //该记录根据用户所属地区分表
        $model->startTrans();
        //1.认证申请库添加数据
...
//        }
    ...
    protected function getMyTenantRoomNum_v1_0_0(){
        ...
        if(!$data){
            ...
            foreach ($data as $k=>$v){
                $province_id =$swf_area->where(['city_code'=>$v['city_id']])->getField('province_code');
                $mode = new Model\GardenRoomModel($province_id,$v['city_id']);
                $num_list[] = $mode->field('id as application_id,real_name,relation_name,room_num,garden_code,city_id')->where(['city_id'=>$v['city_id'],'garden_code'=>$v['garden_code'],'room_num'=>$v['room_num'],'role'=>2])->select();
            }
            $this->echoEncrypData(0,'',$num_list);
        }
    }
    ...
    protected function TenantDelNum_v1_0_0(){
        ...
        if(intval($role) !== 1) {
            ...
            if($res1 and $res3 and $res4 and $res5){
                ...
                $tenant_application->rollback();
                $garden_room->rollback();
                $this->echoEncrypData(1,array($res1 , $res3 , $res4 , $res5),$user_info);
            }
        }
    }
    /*
    ...
    protected function getTenantNumInfo_v1_0_0(){
        ...
        $user_code['user_code']?$city_id =substr($user_code['user_code'],0,6):$city_id=substr($first_application_code,0,6);
        $owner_application = new Model\TenantApplicationModel($city_id);
        $data = $owner_application->where(['city_id'=>$user_code['city_id'],'garden_code'=>$user_code['garden_code'],'room_num'=>$user_code['room_num'],'user_code'=>$user_code['user_code']])->find();
        if(!$data){
            $this->echoEncrypData(1);
        }
    ...
    protected function addGardenMessage_v1_0_0(){
        ...
        if(!$user_info['user_garden']){
            ...
            if(!$res)$this->echoEncrypData(1,$garden_province_id,$garden_city_id);
            $this->echoEncrypData(0);
        }
    }
    /*
    ...
    /*
     * 用户删除认证
     * @param application_id   认证id 认证表的id
