...
//    }
    ...
    public function regiest(){
        ...
        if($res2 and $res3){
            ...
            $point_record->rollback();
            $mongo->baseinfo->user_area->remove(array('account'=>$this->pdata['account']));
            $this->echoEncrypData(1,$point_record->getDbError(),$inviter_code);
        }
    }
...
//        $this->echoEncrypData(1,$data['account_code']);
        ...
        M()->execute('use friends_and_group_'.$data['account_code'].';INSERT INTO `user_friends` ( `user_code`, `nickname`, `portrait`,`friend_user_code`,`friend_nickname`,`friend_portrait`,`group_id`,friend_signature) VALUES ( \''.$data['account_code'].'\', \''.$data['nickname'].'\',\''.$data['portrait'].'\', \''.$data['account_code'].'\', \''.$data['nickname'].'\',\''.$data['portrait'].'\',1,\'该用户太懒，没有留下什么\');use baseinfo;');
        $m =new \MongoClient();
        $baseinfo=$m->baseinfo;
