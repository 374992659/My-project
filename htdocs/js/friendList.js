                    ...
                    if(parseInt(result.errcode)===0){
                        ...
                        if(friends_new_message){              //好友新消息  已按用户分组 时间倒序排列
                            $.each(friends_new_message,function(i,item){
                                ...
                                // 存在本地的聊天记录
                                var json_str = "{'sender_code':'"+item.sender_code+"','type':'"+item.type+"','send_time':'"+item.sender_time+"','content':'"+item.content+"','nickname':'"+item.sender_nickname+"','portrait':'"+item.sender_portrait+"'}";
